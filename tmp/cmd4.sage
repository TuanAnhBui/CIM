" Commands for computing cohomology of degree 4"
from time import time, gmtime, strftime 
logfile = strftime("%d%b%Y%H%M", gmtime()) 
n = 4
with open(os.path.expanduser("./logs/log4.txt"),"w+") as f:
    f.write('LOGFILE FOR THE COMPUTATION OF D'+str(n)+'\n')
total_run_time = time() 
run_time = time() 
load("./tmp/C3_C4.sage")
with open(os.path.expanduser("./logs/log4.txt"),"a+") as f:
...
with open(os.path.expanduser("./logs/log4.txt"),"a+") as f:
    ...
    f.write('It took '+str(time()-run_time)+' seconds!'+'\n')
with open(os.path.expanduser("./results/d4.g"),"w+") as f:    f.write('return '+str(LL)+';') 
with open(os.path.expanduser("./logs/log4.txt"),"a+") as f:
